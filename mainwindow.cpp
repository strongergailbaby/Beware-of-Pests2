    ...
    m_prefs()
{
    ...
    setWindowIcon(QIcon(QPixmap(BIG_PEST_PATH)));
    setWindowTitle(APP);
    newGame();
}
...
void MainWindow::initConnections()
{
    ...
    connect(&m_model,   &CTableModel::pestsDisplay, ui->topWidget, &CTopWidget::setPestsDisplay);
...
void MainWindow::newGame()
{
    ...
    m_model.resetModel(m_prefs.height, m_prefs.width, m_prefs.pests);
    ui->m_view->setModel(&m_model);
    ui->m_view->activate();
...
{
    QMessageBox::about(this, APP, tr("Try to avoid the horrible pests.<br>"));
}
...
void MainWindow::loadSettings()
{
    ...
    m_prefs.pests   = settings.value("pests", int32_t(DEFAULT_PESTS)).toInt();
}
...
void MainWindow::saveSettings()
{
    ...
    settings.setValue("pests", m_prefs.pests);
}
